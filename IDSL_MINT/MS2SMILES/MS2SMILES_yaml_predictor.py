...
def MS2SMILES_yaml_predictor(yaml_file):
    ...
    yaml_data = yaml_data['MINT_MS2SMILES_predictor']
    ...
    if len(deconvoluted_msp) > 0:
        ...
        name_msp_SMILES = ["MSP block name"]
        for i in range(beam_size):
            name_msp_SMILES.append(f"SMILES_{i + 1}")
        name_msp_SMILES = [name_msp_SMILES]
        ...
        if number_processing_threads == 1:
            ...
            for x in tqdm(beam_search_tuple):
                ...
                name_msp_SMILES_job.append(infer_x)
        
        else:
            with torch.inference_mode():
                name_msp_SMILES_job = Parallel(n_jobs = number_processing_threads, timeout = 99999)(delayed(model.beam_search_inference)(x) for x in tqdm(beam_search_tuple))
        
        
        for msp in name_msp_SMILES_job:
            msp_name = [msp[0]]
            for i in msp[2]:
                msp_name.append(i)
            name_msp_SMILES.append(msp_name)

        name_csv = f"{output_directory}/IDSL_MINT_MS2SMILES_prediction.csv"
        ...
        with open(name_csv, "w", newline='') as csvfile:
            ...
            writer.writerows(name_msp_SMILES)
