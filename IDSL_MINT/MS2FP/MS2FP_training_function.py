...
from IDSL_MINT.MS2FP.MS2FP_Dataset import MS2FP_Dataset
from IDSL_MINT.MS2FP.MS2FP_DataLoader import MS2FP_DataLoader
...
def train_step(model: torch.nn.Module, 
    ...
    for MZ_Tokens, INT, FingerPrint, nBitsKeyPaddingMask in dataloader:
        ...
        nBitsKeyPaddingMask = nBitsKeyPaddingMask.squeeze(dim = 0).to(device)
        ...
        pred_fp = predicted_finger_print.reshape(-1, predicted_finger_print.size(-1))
        tgt_fp = target_finger_print.reshape(-1).type(torch.long)

        del MZ_Tokens, INT, nBitsKeyPaddingMask
        
        loss = loss_fn(pred_fp, tgt_fp)
        ...
        train_acc += nBitsTokensTanimotoCoefficient(predicted_finger_print.to('cpu'), input_finger_print.to('cpu'), n_msp_batch)

        del input_finger_print, predicted_finger_print
...
def test_step(model: torch.nn.Module, 
    ...
    with torch.inference_mode():
        for MZ_Tokens, INT, FingerPrint, nBitsKeyPaddingMask in dataloader:
            ...
            test_acc += nBitsTokensTanimotoCoefficient(predicted_finger_print.to('cpu'), input_finger_print.to('cpu'), n_msp_batch)
            del input_finger_print, predicted_finger_print
...
def MS2FP_train(model: torch.nn.Module,
                device: str = None,
    ...
    MS2FP_training = MS2FP_DataLoader(pkl_deconvoluted_msp_directory = pkl_deconvoluted_msp_directory, max_number_ions_per_batch = max_number_ions_per_batch)

    custom_dataset = MS2FP_Dataset(MS2FP_training = MS2FP_training)
    # Set random seeds
    ...
    accuracy_title = "Tanimoto Similarity of Fingerprints [0-100]"
    ...
    for epoch in range(epochs):
        ...
        if train_loss < minimum_train_loss:
            ...
            if epoch != 1:
                torch.save(obj = model.state_dict(), f = f"{output_directory}/MINT_MS2FP_model.pth") # only to save the state_dict() learned parameters
